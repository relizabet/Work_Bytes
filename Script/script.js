$(document).ready(function () {
  ...
  const zomatoApiKey = "ZnAtTuiJDu6IN6Gr7hp9MS2MkxM9hNgT";
  ...
  function findJobs(locationInputCity, locationInputState) {
    ...
    $.ajax({
      ...
      let iterations = response.results.length;
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        let newDiv = $("<div class='input-field new-div-style'></div>")
          ...
          .addClass("jobListingsClick card");
        
        let btnsDiv = $("<div class='input-field new-div-style'></div>")
        // append buttons
        jobListings.append(btnsDiv);
        btnsDiv
        .append(
          `<p class="for-description${i}" style="margin: .5em;"><button class ="btn read-more${i}">Read More</button></p>`
        )
        .append(
          `<button class ="btn restauants style="margin: .5em;"${i}">Nearby Restaurants</button>`
        );
        ...
        let showRestaurants = false;
          $(`button.Restaurants`).on("click", function () {
            ...
            let restaurants = restauantListings;
            if (showRestaurants) {
              findBytes(restaurants);
            } else {
              showRestaurants = !showRestaurants;
              $(`restaurntDiv${i}`)
                .empty()
...
/* 
      ...
      success: function (response) {
        ...
        for (let i = 0; i < response.restaurants.length; i++) {
          ...
          $(`button.Restaurants`).on("click", function () {
            // changes boolean to true
            showRestaurants = !showRestaurants;
            let restaurants = restaurantDiv;
            if (showRestaurants) {
              $(restaurants).append(restauantListings);
            } else {
              showRestaurants = !showRestaurants;
              $(`restaurntDiv${i}`)
                .empty()
          ...
          let restaurantDiv = $("<div class='input-field new-div-style'></div>")
            .attr("restaurant-name", response.restaurants[i].restaurant.name)
            .attr("restaurant-location", response.restaurants[i].restaurant.location)
            .attr("href", response.restaurants[i].restaurant.url)
            .addClass("restaurantListingsClick restaurant-styling card");
          // append restaurant listings
          restaurantListings.append(restaurantDiv);

          restaurantDiv
            .append(`<a class="style-url right">${response.restaurants[i].restaurant.name}</a>`)
            .append(`<i style="float:left;" class="small material-icons">restaurant</i>`)
            .append(
              `<p class="for-menu${i}" style="margin: .5em;"><button class ="btn menu${i}">Menu</button></p>`
            )
            .append(
              `<i style="float:left;" class="small material-icons">location_on</i>`
            );
          ...
          /* let newRestaurant = $(
              `<a class="style-url right">${response.restaurants[i].restaurant.name}</a>`)
  ...
  $(document).on("click", "div.jobListingsClick", function () {
    // get company name
